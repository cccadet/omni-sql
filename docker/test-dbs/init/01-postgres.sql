-- PostgreSQL: dados fictícios para smoke test do omni-sql
-- Roda via docker-entrypoint-initdb.d (executa como usuário POSTGRES_USER = omni)
...
RETURNS NUMERIC AS $$
    SELECT COALESCE(SUM(total), 0) FROM orders WHERE customer_id = p_customer_id;
$$ LANGUAGE sql STABLE;
