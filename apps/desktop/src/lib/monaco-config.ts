        ...
        suggestions: suggestions.map((s, i) => ({
          ...
          insertTextRules:
            s.insertText && (s.insertText.includes("$1") || s.insertText.includes("$2"))
              ...
              : undefined,
          // `!omni:` sorts before normal identifier text generated by Monaco's
          // word-based provider, while the remaining fields preserve relevance
          // and stable ordering among contextual database suggestions.
          sortText: `!omni:${String(relevanceRanks.get(s.relevance) ?? relevanceRanks.size)
