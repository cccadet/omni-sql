...
export const SQL_COMMANDS: readonly SqlCommand[] = [
  ...
  { id: "pg-sessions", title: "Active sessions", description: "Inspect active PostgreSQL sessions and queries.", category: "diagnostics", dialects: ["postgres"], searchTerms: ["sessões", "locks", "queries rodando"], sql: "SELECT pid, usename, state, wait_event_type, wait_event, query_start, query\nFROM pg_stat_activity\nWHERE state <> 'idle'\nORDER BY query_start;" },
  { id: "mysql-processlist", title: "Active sessions", description: "Inspect active MySQL or MariaDB sessions.", category: "diagnostics", dialects: MYSQL, searchTerms: ["sessões", "processos", "queries rodando"], sql: "SHOW FULL PROCESSLIST;" },
  { id: "mssql-sessions", title: "Active requests", description: "Inspect SQL Server requests currently executing.", category: "diagnostics", dialects: ["sqlserver"], searchTerms: ["sessões", "requests", "queries rodando"], sql: "SELECT session_id, status, command, wait_type, start_time\nFROM sys.dm_exec_requests\nWHERE session_id <> @@SPID;" },
  { id: "oracle-sessions", title: "Active sessions", description: "Inspect active Oracle user sessions.", category: "diagnostics", dialects: ["oracle"], searchTerms: ["sessões", "locks", "queries rodando"], sql: "SELECT sid, serial#, username, status, event, sql_id\nFROM v$session\nWHERE type = 'USER' AND status = 'ACTIVE';" },
];
