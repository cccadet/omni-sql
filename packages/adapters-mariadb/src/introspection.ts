...
import { mariadbDescriptor, quoteIdentifier } from "@omni-sql/dialect-descriptors";

// MariaDB fala o mesmo protocolo de fio do MySQL (driver `mysql2` funciona
// para os dois) e expõe o mesmo `information_schema` — este arquivo é quase
// idêntico a `adapters-mysql/src/introspection.ts`, só troca o descritor de
// dialeto usado para quoting de identificador.
...
// Schemas mantidos pelo próprio MariaDB — nunca interessam ao usuário final.
...
// o `information_schema` do MariaDB devolve o nome da coluna em MAIÚSCULO no
...
/** Lista os nomes de schema (= database no MariaDB) disponíveis — usado pela UI para deixar o usuário escolher o que indexar. */
...
// MariaDB não tem overload de função (uma assinatura por nome).
const FUNCTIONS_SQL = `
...
// PRIMARY é o nome fixo do índice de PK no MariaDB — sinaliza `primary` sem
...
export async function getDefinitionViaPool(
  ...
  const ref = `${quoteIdentifier(mariadbDescriptor, schema)}.${quoteIdentifier(mariadbDescriptor, name)}`;
  if (kind === "view") {
...
/**
 * MariaDB não tem um análogo direto ao cursor server-side do Postgres via
...
export async function updateRowViaPool(pool: Pool, spec: RowUpdateSpec): Promise<number> {
  ...
  const setClause = setEntries
    .map(([col, val]) => {
      values.push(val);
      return `${quoteIdentifier(mariadbDescriptor, col)} = ?`;
    })
    .join(", ");
  ...
  const tableRef = spec.schema
    ? `${quoteIdentifier(mariadbDescriptor, spec.schema)}.${quoteIdentifier(mariadbDescriptor, spec.table)}`
    : quoteIdentifier(mariadbDescriptor, spec.table);
