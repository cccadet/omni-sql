...
import type { Adapter, TestResult } from "./index.ts";

/**
 * Adaptador in-memory para Fase 0: sem dependência externa (sem pg, sem
 * docker). Suporta `SELECT 1` (echo), uma tabela sintética `users` e algumas
 * colunas — suficiente para destravar Monaco + results grid + smoke E2E.
 *
 * Em Fase 2 este arquivo permanece, mas surge `packages/adapters-core/src/pg.ts`
 * com introspecção real via `pg`.
 */
